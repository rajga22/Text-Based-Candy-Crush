...
while (true) {
printGrid();
int row1 = getValidCoordinate(scanner, "first candy row");
int col1 = getValidCoordinate(scanner, "first candy column");
int row2 = getValidCoordinate(scanner, "second candy row");
int col2 = getValidCoordinate(scanner, "second candy column");

if (Math.abs(row1 - row2) + Math.abs(col1 - col2) == 1) {
swapCandies(row1, col1, row2, col2);
if (!processMatches()) {
System.out.println("No match was made. Try again.");
swapCandies(row1, col1, row2, col2); // Swap back if no match
}
} else {
...
private int getValidCoordinate(Scanner scanner, String coordinateType) {
while (true) {
...
System.out.println("Invalid " + coordinateType + ". It must be between 0 and 7.");
}
} else {
System.out.println("Invalid input. Please enter a number.");
scanner.next(); // Consume the non-integer input to clear the buffer
}
}
}
...
private boolean checkForMatch(int i, int j) {
char candy = grid[i][j];
int horizMatch = countMatch(i, j, 0, 1);
int vertMatch = countMatch(i, j, 1, 0);
if (candy != ' ' && (horizMatch >= 3 || vertMatch >= 3)) {
int matchedCandies = Math.max(horizMatch, vertMatch);
score += matchedCandies * 10; // Increase score for each candy matched
clearAndCascade(i, j, candy == BOMB);
return true;
}
return false;
}

private int countMatch(int row, int col, int dRow, int dCol) {
...
private void clearAndCascade(int row, int col, boolean isBomb) {
if (isBomb) {
for (int i = 0; i < SIZE; i++) {
...
clearVertical(row, col);
clearHorizontal(row, col);
}
fillEmptySpaces();
}

private void clearVertical(int row, int col) {
...
private void clearHorizontal(int row, int col) {
